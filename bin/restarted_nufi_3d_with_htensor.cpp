...
const size_t Nx = htensor::n_r;  // Number of grid points in physical space.
...
/*     if(x < 0 || x > htensor::Lx){
    ...
    z = std::fmod(std::fmod(z, htensor::Lz) + htensor::Lz, htensor::Lz);
    size_t z_ref_pos = std::min(static_cast<size_t>(std::floor(z / htensor::dz_r)), htensor::n_r - 1);
    ...
    size_t w_ref_pos = std::min(static_cast<size_t>(std::floor((w-htensor::wmin)/htensor::dw_r)), htensor::n_r - 1);
