...
template <typename real, size_t order>
real eval_f_nufi_ham_strang_HE_HB_Hf_HB_HE_1x2v(
    size_t n, real x, real u, real v,
...
{
    // Convention:
    //   eval_f_tilde_strang_1x2v(n, ...) returns the partially advanced state
    //   entering the H_f-step of the nth full update, i.e.
    //
    //     \tilde f^{n-1} = Phi_HE(dt/2) Phi_HB(dt/2) f^{n-1}
    //
    // Hence this routine is valid only for n >= 1 and uses fields at level n-1.

    if (n == 0) {
        throw std::invalid_argument(
            "eval_f_tilde_strang_1x2v: n must be >= 1 under the convention that j_n uses fields from time level n-1."
        );
    }
    ...
    // 2) Undo H_E(dt/2) using fields at level n-1
    {
    ...
    return eval_f_nufi_ham_strang_HE_HB_Hf_HB_HE_1x2v<real, order>(
        n - 1, x, u, v,
    ...
    for (size_t iu = 0; iu < conf.Nu; ++iu) {
        ...
        for (size_t iv = 0; iv < conf.Nv; ++iv) {
            ...
            for (size_t ix = 0; ix < Nx; ++ix) {
                ...
                const double f_tilde = eval_f_tilde_strang_1x2v<double, order>(
                            n_state, xpos, u, v,
                            coeffs_Ex, coeffs_Ey, coeffs_Bz,
                            conf
                        );
            ...
            for (size_t m = 0; m < Nx; ++m) {
                ...
                // j_x kernel:
                // integral_0^dt u exp(-i k u s) ds
                // = (1 - exp(-i k u dt)) / (i k),  k != 0
                double Kx_re, Kx_im;
                if (std::abs(k) < eps) {
                    Kx_re = u * dt;
                    Kx_im = 0.0;
                ...
                // j_y kernel:
                // integral_0^dt v exp(-i k u s) ds
                // = v (1 - exp(-i k u dt)) / (i k u),  ku != 0
                double Ky_re, Ky_im;
                if (std::abs(ku) < eps) {
                    Ky_re = v * dt;
                    Ky_im = 0.0;
...
template <typename real, size_t order>
void eval_rho_ham_strang_HE_HB_Hf_HB_HE_1x2v(
    size_t n,
    std::vector<real>& rho,
    const std::vector<real>& coeffs_Ex,
    const std::vector<real>& coeffs_Ey,
    ...
    const config_t<real>& conf)
{
    #pragma omp parallel for
    for (size_t l = 0; l < conf.Nx; ++l) {
        const real x = conf.x_min + l * conf.dx;

        real sum0 = real(0);

        for (size_t iu = 0; iu < conf.Nu; ++iu) {
            for (size_t iv = 0; iv < conf.Nv; ++iv) {
                const real u = conf.u_min + (iu + real(0.5)) * conf.du;
                const real v = conf.v_min + (iv + real(0.5)) * conf.dv;

                const real f = eval_f_nufi_ham_strang_HE_HB_Hf_HB_HE_1x2v<real, order>(
                    n, x, u, v,
                    coeffs_Ex, coeffs_Ey, coeffs_Bz, conf
                );
...
template <typename real, size_t order>
void eval_j_full_EBf_MPI(
    size_t n,
    std::vector<real>& j,
    const std::vector<real>& coeffs_E,
    const std::vector<real>& coeffs_B,
    const config_t<real> &conf )
{
    int rank, size;
    ...
    // -------------------------
    // balanced decomposition of configuration space
    // -------------------------
    size_t base = Nspace / size;
    size_t rem  = Nspace % size;

    size_t Nloc = base + (rank < rem ? 1 : 0);
    size_t l0   = base * rank + std::min((size_t)rank, rem);
    size_t lend = l0 + Nloc;

    // -------------------------
    // MPI-parallel configuration loop
    // -------------------------
    for(size_t l = l0; l < lend; l++)
    {
        ...
        real pref = conf.q * conf.du * conf.dv * conf.dw;

        j[l]                         = pref * sum0;
        j[l + Nspace]                = pref * sum1;
        j[l + 2*Nspace]              = pref * sum2;
    }
}
...
template <typename real, size_t order>
void eval_rho_full_EBf_MPI(size_t n, std::vector<real>& rho, const std::vector<real>& coeffs_E, 
    const std::vector<real>& coeffs_B, const config_t<real> &conf )
{
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
