...
{
	return nufi::dim3::eval<real, order, dx, dy, dz>(x, y, z,
...
void electro_magnetic_force<real>::solve_phi(real* rho_phi, bool save_result)
{
    ...
    if(save_result)
    {
        nufi::dim3::interpolate(coeffs_phi.get() + curr_tn*stride_t, rho_phi, param);
    }
}
...
void electro_magnetic_force<real>::solve_A(real* j_A_i, size_t index, bool save_result)
{
    ...
    if(save_result)
    {
    	...
    	{
    	case 1:
            nufi::dim3::interpolate<real,order>(coeffs_A_x.get() + curr_tn*stride_t, j_A_i, param);
            break;
    	case 2:
    		nufi::dim3::interpolate<real,order>(coeffs_A_y.get() + curr_tn*stride_t, j_A_i, param);
        	break;
    	case 3:
    		nufi::dim3::interpolate<real,order>(coeffs_A_z.get() + curr_tn*stride_t, j_A_i, param);
        	break;
    	}
    }
}
