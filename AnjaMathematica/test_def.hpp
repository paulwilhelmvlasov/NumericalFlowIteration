...
real eval_f( size_t n, real x, real y, real z, real u, real v, real w, const real *coeffs, const config_t<real> &conf ){
    ...
    return 1.0 / (4.0 * M_PI * sqrt(3.0)) * (exp(-(pow(u + sqrt(2), 2) + pow(v, 2) + pow(w + sqrt(3.0/2.0), 2))) +
        exp(-(pow(u - 2*sqrt(2), 2) + pow(v, 2) + pow(w - sqrt(3.0/2.0), 2))) + exp(-(pow(u, 2) + pow(v + 3*sqrt(2), 2) + pow(w, 2))) +
        exp(-(pow(u, 2) + pow(v - 4*sqrt(2), 2) + pow(w, 2))));
}